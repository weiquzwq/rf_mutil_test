...
def check_grid(url):
    ...
    while(flag and ct<10):
        try:
            r = requests.get(url='{0}/favicon.ico'.format(url))
            if r.status_code==200:
                print "the selenium-grid respone code is {0}".format(r.status_code)
                flag=0
                return 1
            else:
                ct=ct+1
            ...
            ct=ct+1
            print "selenium-grid is not start by :{0},try to check again now ,time:{1}".format(url,ct)
            time.sleep(3)
