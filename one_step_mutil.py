...
if plat in ["Mac"]:
  if testos in ["iOS","ios"]:
    ...
    run_test="python robot_mutil_test.py -s {0} -t {1} -o {2} -p {3} -sp {4}".format(testsuite,tags,testos,iport,wport)
  else:
    run_server="python run_server.py -o {0} -p {1} -sp {2} -n {3}".format(testos,aport,bport,num)
    run_test="python robot_mutil_test.py -s {0} -t {1} -o {2} -p {3} -sp {4}".format(testsuite,tags,testos,aport,bport)
